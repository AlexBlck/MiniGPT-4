...
class MiniGPTBase(BaseModel):
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompts, lengths=None):
        if prompts is None or len(prompts) == 0:
            ...
            for idx, (each_img_embed, each_prompt) in enumerate(
                zip(img_embeds, prompts)
            ):
                pn = each_img_embed.shape[-2]
                if lengths is not None:
                    each_img_embed = each_img_embed.reshape(
                        -1, each_img_embed.shape[-1]
                    )
            ...
            max_length = (
                max(emb_lens)
                if max(emb_lens) < self.max_context_len
                else self.max_context_len
            )
            wrapped_embs = pad_emb.expand(len(emb_lens), max_length, -1).clone()
            wrapped_atts = torch.zeros(
                [len(emb_lens), max_length], dtype=torch.int, device=img_embeds.device
            )

            for i, emb in enumerate(emb_lists):
                length = (
                    emb_lens[i]
                    if emb_lens[i] < self.max_context_len
                    else self.max_context_len
                )
    ...
    def tokenize_conversation(self, conv_q, conv_a):
        ...
        for batch_idx in range(batch_size):
            ...
            questions = [
                self.llama_tokenizer(
                    self.llama_tokenizer.bos_token + q,
                    return_tensors="pt",
                    add_special_tokens=False,
                ).to(self.device)
            ...
            answers = [
                self.llama_tokenizer(
                    a + self.end_sym, return_tensors="pt", add_special_tokens=False
                ).to(self.device)
                for a in answers
            ]
            cur_id = []
        ...
        to_regress_token_attn = (
            to_regress_token_ids != self.llama_tokenizer.pad_token_id
        ).to(torch.int)
    ...
    def preparing_embedding(self, samples):
        ...
        if "conv_q" in samples:
            ...
            conv_q = [
                [self.prompt_template.format(item) for item in items]
                for items in conv_q
            ]

            cond_embeds, cond_atts = self.prompt_wrap(
                img_embeds, img_atts, [q[0] for q in conv_q]
            )
            regress_token_ids, regress_atts, part_targets = self.tokenize_conversation(
                conv_q, conv_a
            )
            ...
            if hasattr(self, "chat_template") and self.chat_template:
                instruction = [
                    self.prompt_template.format(instruct) for instruct in instruction
                ]
    ...
    def forward(self, samples, reduction="mean"):
        ...
        for i, target in enumerate(part_targets):
            targets[
                i, input_lens[i] + 1 : input_lens[i] + len(target) + 1
            ] = target  # plus 1 for bos
    ...
    def generate(
        ...
        for output_token in outputs:
            ...
            output_texts = self.llama_tokenizer.decode(
